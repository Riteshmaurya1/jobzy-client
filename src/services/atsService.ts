...
export const atsService = {
    ...
    check: async (formData: FormData): Promise<any> => {
        const res = await fetch(`${API_BASE_URL}/ats/check`, {
            method: "POST",
    ...
    getKeywordSuggestions: async (formData: FormData): Promise<any> => {
        const res = await fetch(`${API_BASE_URL}/ats/keywords/suggestions`, {
            method: "POST",
            headers: getAuthHeader(),
            body: formData
        });
    ...
    matchKeywords: async (resumeText: string, jobDescription: string): Promise<any> => {
        const res = await fetch(`${API_BASE_URL}/ats/keywords/match`, {
            method: "POST",
            headers: getHeaders(),
